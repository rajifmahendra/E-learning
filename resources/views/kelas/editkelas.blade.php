...
<div class="card">
  <h1 class="text-primary card-header"><b>Edit Data Kelas</b></h1><br>
  <form class="form-horizontal" method="POST" action="{{'/kelas/'.$data_kelas->id}}">
    {{ method_field('PATCH') }}
    @csrf
    <div class="form-group">
      ...
      <tr class="has-feedback{{ $errors->has('html_kelas') ? 'has-error' : '' }}">
        ...
        <td><input class="form-control" type="text" name="html_kelas" value="{{ $data_kelas->kelas }}" readonly>
      ...
      <tr>
            ...
            @foreach ($data_walikelas as $value)
              @if($value->id == $data_kelas->master_data_id)
